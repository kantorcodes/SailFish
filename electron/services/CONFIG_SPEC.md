...
- **问题**：整窗放大缩小只能靠快捷键，很多人不知道；焦点落在预览网页里时，快捷键会改错对象或看起来没反应；改完一重启又回到默认。
- **成功标准**：设置里能看见、能改。改完立刻整窗变大变小，和菜单「放大 / 缩小 / 实际大小」是同一件事。关掉再开还在。不管焦点在终端、预览还是对话框，快捷键改的都是应用界面本身。
- **取舍**：沿用现有菜单快捷键，不为缩放再发明一套。终端字号仍然单独调，不被这项取代。
- **不做**：只改字不改间距的另一套字号；不把预览网页里的缩放和整窗缩放混成一件事。

### 消息输入快捷键

- **问题**：发消息和排队的按键是写死的。写长文容易误发；习惯用组合键发送的人改不了。设置里能改别的快捷键，偏偏改不了输入框这几条。
- **成功标准**：设置 → 快捷键能自己录「发送」和「排队到任务结束后再做」。改完立刻生效。输入框上的提示跟着实际按键走。没被这两条占用的回车（包括 Shift+回车）用来换行。
- **取舍**：这两条只在消息输入框里生效，不进菜单、不当全局快捷键，避免回车在整个应用被抢走。发送和排队如果会抢同一下按键，设置里要拦下来。
- **不做**：不单独设换行快捷键；不改首次引导里那小段对话、不改远程网页会话、不改文件管理器的传输按键。
