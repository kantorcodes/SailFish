...
> Last verified: 2026-09-11（撑爆后别原样重发；相同大段别再灌一遍）
...
>   7. 真的开始压缩时，过程出现在这场任务的过程里，像它自己调工具一样：那一行说「正在压缩上下文」，做完还在那儿。不要用飘走的提示报进度或结果。
> - **关键取舍**：不另做一套压缩。人点名的时候只是打开同一扇门，门槛从「窗口快满」改成「人说了算」。写什么、留什么，靠已经有的那份交接说明，相信它能写清楚。
> - **明确不做**：不因此清空对话；不在它干活时硬压；不为了省事按远近砍原文。正在干活才说一声等结束；空对话才说没什么可压。人已经点名之后，不要再拦一道「范围够不够」。
>
...
> - **关键取舍**：任务记得「这场都说了什么」靠原文。整场已经塞满，靠交接。交接是检查点，重开接着用，不把压掉的再展开。任务不另做一套按远近收过程的档位。
...
>   2. 模型任何时候都能主动问出「还剩多少空间」，不必等告警推到面前。这件事不靠每轮往对话里塞一个数字来解决：那些数字会永久沉淀成一串过期读数，既占地方，又让模型分不清哪个是此刻的。
...
> - **成功标准**：它说给用户听的话永远在外面，它做事的过程永远收着一行；那行从头到尾位置不动，跑着时说在忙什么并走秒，做完说做了什么、花了多久；用户插话打断正在做的事，那一行立刻停表、改口说做了什么，不再转圈计时；过程中某次工具失败收进去，只有整件事没办成才告诉用户；要用户动手的（确认、提问、密码）永远在外面；任何任务都收，形态始终一致；点开能看到底稿，也可以在设置里整个关掉。
...
>   3. **尽早失败**：路径已经完整说出、且能确定「以新建方式写已存在文件」时，立即中止当前生成并给出明确错误，不等后面的文件内容写完。路径还在往外吐、只写出前半截时，即使那一截碰巧是已有文件夹，也不能当成目标去判失败（否则会把「用户目录 / 应用数据目录」这类前缀误报成写错地址）。
> - **关键取舍**：保留「新建撞已存在即失败」的防误覆盖语义，不自动升级为覆盖；早失败只缩短弯路、不替模型改决定。早失败通过工具元数据声明，抽象层不感知具体工具名。半截路径不算「能确定」。
