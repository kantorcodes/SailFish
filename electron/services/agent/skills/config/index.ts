...
const configSkill: Skill = {
  id: 'config',
  name: '旗鱼配置管理',
  description: '读取和修改旗鱼应用配置（界面语言、主题、终端设置、Agent 性格、IM/邮件账号连接凭证等），管理邮箱/日历账户（添加、删除、验证连接），测试 IM 连接，为旗鱼添加/更新/删除 AI 模型配置（config_ai_profile；列表能看见名称、模型名、地址、是否已填 Key，不回显 Key），管理联网搜索服务商（config_web_search；能看清当前用哪家、密钥填了没有，可切换、填密钥、改档位，不回显密钥，改完立刻生效），添加/更新/删除 SSH 主机（config_ssh_session；一条一条来，可写密码但不回显），以及添加/更新/删除 MCP 连接器（config_mcp_server_add/update/delete；保存启用后会自动连接，无需重启）。适用于用户要求调整设置、添加主机、添加 AI 模型、配置邮箱/日历、检查集成状态、配置 IM 机器人、接入 MCP 工具服务等场景。不负责用户自定义技能的 API Key（加载 skill-manager 技能，用 skill_set_env 工具处理）。',
  tools: configTools,
