...
- 多维表格筛选使用 JSON 格式 filter 参数`,
    ...
    'hint.command_too_long': '命令过长（{length} 字符，限制 {max}），终端会截断。请先用 write_text_file/write_remote_text_file 写入脚本文件再执行。',
    ...
    'error.write_mode_required': '缺少必填的写入方式 mode 参数，支持: {modes}。新建用 create（目标已存在会失败），整文件重写用 overwrite，追加用 append',
...
- Bitable filter uses JSON format with conditions`,
    ...
    'hint.command_too_long': 'Command too long ({length} chars, limit {max}), terminal will truncate. Please use write_text_file/write_remote_text_file to create a script file first.',
    ...
    'error.write_mode_required': 'Missing required mode parameter. Supported: {modes}. Use create to create new (fails if target exists), overwrite to rewrite whole file, append to append',
