...
- 多维表格筛选使用 JSON 格式 filter 参数`,
    ...
    // 任务管理（manage_tasks，仅联络 Agent）
    'taskmgr.step': '任务管理',
    'taskmgr.not_available': '任务管理能力不可用',
    'taskmgr.unknown_action': '未知的 action：{action}，支持 list / inspect / create / confirm / stop',
    'taskmgr.missing_param': '缺少参数：{param}',
    'taskmgr.task_not_found': '未找到任务：{taskId}。先用 list 查看当前任务的 task_id。',
    'taskmgr.action.list': '查看任务列表',
    'taskmgr.action.inspect': '查看任务近况',
    'taskmgr.action.create': '创建任务',
    'taskmgr.action.confirm': '传达确认决定',
    'taskmgr.action.stop': '停止任务',
    'taskmgr.list_empty': '当前没有任务。',
    'taskmgr.list_header': '当前任务（{count} 个，待确认优先）：',
    'taskmgr.status_pending': '等待确认',
    'taskmgr.status_running': '运行中',
    'taskmgr.status_idle': '空闲',
    'taskmgr.line_status': '状态：{status}',
    'taskmgr.line_pending': '待确认：{tool}（{risk}，tool_call_id={toolCallId}）｜参数：{args}',
    'taskmgr.line_running': '当前工具：{tool}｜已执行 {steps} 步',
    'taskmgr.line_idle': '已执行 {steps} 步',
    'taskmgr.inspect_header': '任务「{title}」（task_id={taskId}）',
    'taskmgr.inspect_meta': '形态：{terminalType}｜状态：{status}｜已执行 {steps} 步',
    'taskmgr.inspect_pending': '待确认：{tool}（{risk}，tool_call_id={toolCallId}）\n参数：{args}',
    'taskmgr.inspect_recent': '最近步骤（最近 {count} 条，每条已截断）：',
    'taskmgr.created': '已创建任务「{title}」（task_id={taskId}），正在开始执行（{mode}）。任务完成或需要确认时会通知你；期间可用 list / inspect 查看进度。',
    'taskmgr.mode_free': '全自动',
    'taskmgr.mode_relaxed': '危险操作需确认',
    'taskmgr.approve': '批准',
    'taskmgr.reject': '拒绝',
    'taskmgr.confirm_done': '已将用户的「{decision}」传达给任务「{title}」。',
    'taskmgr.confirm_stale': '该确认已不存在（可能用户已在任务卡片上处理，或任务已继续）。可用 list 查看最新状态。',
    'taskmgr.stopped': '已停止任务「{title}」。',
    'taskmgr.not_running': '任务「{title}」当前未在运行。',
  },
  'en-US': {
    // Tool names
...
- Bitable filter uses JSON format with conditions`,
    ...
    // Task management (manage_tasks, companion only)
    'taskmgr.step': 'Manage tasks',
    'taskmgr.not_available': 'Task management is not available',
    'taskmgr.unknown_action': 'Unknown action: {action}. Supported: list / inspect / create / confirm / stop',
    'taskmgr.missing_param': 'Missing parameter: {param}',
    'taskmgr.task_not_found': 'Task not found: {taskId}. Use list to get current task_ids.',
    'taskmgr.action.list': 'List tasks',
    'taskmgr.action.inspect': 'Inspect task',
    'taskmgr.action.create': 'Create task',
    'taskmgr.action.confirm': 'Relay confirmation',
    'taskmgr.action.stop': 'Stop task',
    'taskmgr.list_empty': 'No tasks right now.',
    'taskmgr.list_header': 'Current tasks ({count}, pending confirmation first):',
    'taskmgr.status_pending': 'awaiting confirmation',
    'taskmgr.status_running': 'running',
    'taskmgr.status_idle': 'idle',
    'taskmgr.line_status': 'status: {status}',
    'taskmgr.line_pending': 'pending: {tool} ({risk}, tool_call_id={toolCallId}) | args: {args}',
    'taskmgr.line_running': 'current tool: {tool} | {steps} steps',
    'taskmgr.line_idle': '{steps} steps',
    'taskmgr.inspect_header': 'Task "{title}" (task_id={taskId})',
    'taskmgr.inspect_meta': 'type: {terminalType} | status: {status} | {steps} steps',
    'taskmgr.inspect_pending': 'Pending: {tool} ({risk}, tool_call_id={toolCallId})\nArgs: {args}',
    'taskmgr.inspect_recent': 'Recent steps (last {count}, each truncated):',
    'taskmgr.created': 'Task "{title}" created (task_id={taskId}), starting now ({mode}). You will be notified when it finishes or needs confirmation; use list / inspect anytime for progress.',
    'taskmgr.mode_free': 'fully automatic',
    'taskmgr.mode_relaxed': 'dangerous ops need confirmation',
    'taskmgr.approve': 'approve',
    'taskmgr.reject': 'reject',
    'taskmgr.confirm_done': 'Relayed the user\'s "{decision}" to task "{title}".',
    'taskmgr.confirm_stale': 'That confirmation no longer exists (the user may have handled it on the task card, or the task moved on). Use list for the latest state.',
    'taskmgr.stopped': 'Task "{title}" stopped.',
    'taskmgr.not_running': 'Task "{title}" is not running.',
  }
} as const
