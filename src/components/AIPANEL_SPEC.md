...
> Last verified: 2026-09-10（悬停消耗看出速）
...
> - **成功标准**：
>   1. 悬停底栏那个消耗数字，在输入、输出后面能看到速度，单位是 tokens/s。
>   2. 正在往外吐字时写「出速」，数字是这一段从开口算起有多快。
>   3. 停住不吐了，改成「均速」，数字是刚结束那一段的平均。再开始吐字时回到出速。
>   4. 还没开始吐字时不出现这一项。
> - **关键取舍**：速度跟在账后面，不占底栏。出速看当下这一段，均速看刚停住的那一段，不为整场另算一个总均速。
> - **明确不做**：不把速度钉在底栏数字旁边；不把工具执行、思考停顿算进速度里。
