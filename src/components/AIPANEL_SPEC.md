...
>   3. Tab 补全名字。回车在命令还没写完整时也是补进输入框，不立刻执行；写完整后再回车才执行，不当成一句对话发出去。
>   4. 这一版只提供压缩：`/compact`，中文 `/压缩` 也认。
>   5. 真的开始压缩时，过程跟它自己调工具一样出现在这场任务的过程里：那一行说「正在压缩上下文」，点开能看见这一步。命令本身仍不画成用户气泡。
>   6. 它正在干活时不硬压。普通回车要说清楚这场还在跑、可以用 ⌘/Ctrl+回车排队；⌘/Ctrl+回车跟普通话一样进同一条队列，这场结束后再压。进了队列的要看得见。排队到点了仍按命令去压，不当成一句聊天；正在干活时也不能把这条插进当前这场。
