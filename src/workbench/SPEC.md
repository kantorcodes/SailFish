...
- **终端页窗格可拖着排放，边上用来新建（2026-09-10 确认）**：已经并排的几扇，抓住顶上的机器名拖到另一扇边上换位置。从主机列表或新建按钮拖到眼前窗口边上，松手就在那边新开。不要把已打开的标签拖进去合并。这套只给终端页，助手对话页不做。不做成浮动、重叠或独立系统窗口。同一方向再加一扇要均分，不要一层层对半切；换方向才在那一格里劈开。拖主机过来先出窗格、显示正在连接，连不上也在那一扇里说，不要干等再弹超时。
