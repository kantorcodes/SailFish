...
> Last verified: 2026-09-12（HTML 预览超出窗口要能滚、能拖）
...
**HTML 产出物的浏览器形态**（2026-08-04 确认，2026-09-05 补）：HTML 类产出物的预览不是静态图片式的「看结果」，而是应用内浏览器——可刷新、可访问助手启动的本地开发服务（live URL）、可把渲染结果截图连同用户意见回传给助手修改，形成「看效果 → 提意见 → 助手改 → 再看」的闭环。文件预览还可以划一段字，让助手只改这段（见下方）。明确不做：页面上钉区域评论；面板预览与用户真实浏览器无关，不接入 browser-bridge；一个产出物一个预览实例，不做多标签浏览器。
...
**网页预览超出窗口**（2026-09-12 确认）：网页比眼前这块大时，必须能看完——要有滚动条，也可以按住拖着挪，触控板滑动同样能移。不要把多出来的裁掉让人够不着。幻灯片预览和助手起的实时网页不按这个办（它们自己会滚，或本来就是完整网页）。划一段字让助手改的能力还在：拖空白处是挪画面，在字上划仍是选字。
...
**选中后的提示（2026-08-20 确认，2026-09-05 补）**：光有选区高亮，用户不知道选完还能干什么。所以划完一段后，在选区旁边浮一条小提示。Word / WPS / Markdown 写「让旗鱼改这段：提要求，或右键」；HTML 文件预览没有右键快捷菜单，提示改成「让旗鱼改这段：提要求即可」。提示和右键菜单标题都直接叫助手的名字，不叫「AI」；用户给助手起过名就叫用户起的那个（这个名字全应用一个口径）。提示不提「上下左右」哪个方向（对话与预览的相对位置会变，说方位就会指错）。它不占预览高度，划选时预览不跳；不挡住选中的正文；另划一段就跟着走；开始打字、点到输入框、取消选区就消失。不做常驻底栏提示。

**HTML 文件预览选区（2026-09-05 确认）**：HTML 文件预览和 Word 一样，用户划一段就能让助手只改这段。预览仍然只读——不在页面里直接打字改（预览是给人看的译本，写不回原文件还不毁结构）。不要右键快捷菜单：划完自己去输入框写要求。行为承诺：

- **选中就能发指令**：划完直接在下方输入框写要求；选区高亮在点到输入框后仍保留，方便确认范围
- **不出现引用胶囊，聊天气泡也不展示选区脚手架**：只留用户写下的话，选中原文经工作台上下文旁路交给助手
- **按选中原文定位**：没有精确行号，助手以选中文字为锚；预览里的字和源文件偶尔对不齐时，助手应再读文件对准，而不是改错地方
- **改完立刻看见**：助手改完这份已打开的文件后，预览立刻换成新内容，不用关开。预览仍只读，磁盘文件是真相
- **整页截图还在**：改布局、配色、整页观感仍用截图加意见

明确不做：在预览里直接打字改页面；改写/润色/校对等右键快捷菜单；PPT 预览划选（仍另案）；live URL（助手起的本地服务那种）划选——页面会跳、源码也不一定是眼前这份文件。

**面板收起（2026-08-17 确认）**：产出物入座时，收起与历史对话侧栏同一套手感——收起后整栏消失，不留一条图标窄栏。开关钉在工作台右上角，开合时不跟着面板跑；有产出物且文件在座位上（或可以入座）时在，图标随开合换向。助手再产出新东西时，若座位空着或正坐着文件，面板会自动打开；若终端正在座位上，新文件只进清单、不抢座。开合是抽屉推拉：整栏变速滑入滑出，不是整块闪没或闪现。

**面板宽度（2026-08-27 确认）**：用户拖分隔条调出的是相对比例。调整窗体大小时，产出物面板按这个比例跟着走，不要等停稳再滑。历史侧栏开合时仍保持当时看到的宽度，不跟着工作台变窄变宽再算一遍。

**产出物清单（2026-08-17 确认，2026-08-19 补，2026-08-21 改，2026-08-22 补，2026-08-27 改）**：有产出物时，清单按钮钉在对话区右上角。右上角的清单和折叠开关靠右排成一组，有几个就显示几个，不为没出现的按钮留空位。产出物栏开着时清单停在分隔线旁，折叠钉在画布最右侧；栏收着时两者贴在一起靠右。终端入座时清单停在对话折叠左边（对话折叠实际出现才让位）。文件都清了、但终端还在桌上且不在座位上时，清单仍在，方便请终端回来；正在看终端且没有文件时，清单不必出现。鼠标移到清单按钮上就展开一块实底小面板，不必再点；移开按钮和面板就收起。展开时盖在对话区上面（包括计划进度条），不被挡住。宽度随名称长短伸缩，进出带一点弹性。桌上还有终端时，清单第一项是这扇终端，其余是文件。清单是这场对话产出过什么的目录，一直在；点一项就在面板里打开这份，清单上不关文件。要从这场对话里拿走一份，只在清单里移出，说法是「从清单移出」，不说「桌上」——界面别处没有这张桌子，单独冒出来对不上。不靠鼠标贴边自动弹出。
...
**Excel 预览选区（2026-08-27 确认）**：Excel 预览里用户圈一块格子就能让助手只改这块。预览仍然只读——不在格子里直接打字（预览是给人看的译本，写不回原文件还不毁格式）。表不是文档：圈完自己去输入框写要求就行，不要 Word 那套悬浮提示和右键快捷菜单。行为承诺：
...
- **改完立刻看见**：助手改完这份已打开的表后，预览立刻换成新内容，不用关开。刚改过的格子亮大约一秒再淡掉，方便对一眼，不要常亮。预览仍只读，磁盘文件是真相
...
**文档铺满（2026-08-25 确认，2026-09-04 补）**：文件默认仍是右边参照。读长文、改一章、看宽表时，人可以把这份文件临时变成工作面——文档占满工作区，对话流藏起，输入换成一颗浮在文档最底下的薄胶囊，文档上浮一句它刚说的或它在忙什么。要你确认、回答、输密码时，这句话让开，完整出现。这不是第三种座位，只是文件在座时多一种看法。

- **默认分栏**：短回复、几行稿子不自动铺满。不按类型、字数、标题猜测。
- **人点才进**：文档顶栏同一颗按钮进出铺满，图标是一对，位置不变。铺满后不另出一颗收起面板，免得当成返回。助手新写出东西：已经在铺满就留在铺满并换成这份；在分栏就还在分栏。换文件仍可用清单。
- **只在这一次入座期间记住**：换页签、点清单换文件，保持铺满。收起或关光页签再打开，回到分栏。终端入座时铺满结束。不写进历史，不跨会话。
- **浮层就是那句话**：没有「最近一条」标题，不贴文件卡片、不点赞。做完后默认一行，太长省略，点开看完整，再点收起。运行中要看得见它在做什么——就是对话里那一行过程，点开能看到步骤，不另做一套。只看这一次启动之后的，不翻更早的历史。提问回答完继续忙，还是回到这一行，不要留下一块空面板。回分栏只走顶栏按钮。还没说过话、也不在忙，浮层不出现。不另写摘要。
- **输入浮在文档上（2026-09-04 确认）**：还是原来那套输入，只换一副薄胶囊的皮，盖在文档最底下，不另占一条座位。铺满时只要打字和发送——技能、型号、用量这些先藏着，人在改稿，不是在调设置。空着时一条；打字、附件、排队才变高。它刚说的那句仍浮在这条上面。划选改文档、排队、过程那一行的口径都不改。贴文件、快捷键仍可用。
- **明确不做**：系统全屏、藏侧栏；为铺满另写一句漂亮人话；把终端也改成一句浮层；铺满偏好写进设置；另做一套输入。
...
- **宽度**：拖分隔条记下的是比例；调整窗体大小时按这个比例跟着走。历史侧栏开合时宽度保持原样
